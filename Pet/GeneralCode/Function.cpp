...
void Function::ConsoleShowQPoint(const QPoint& _point,const std::string& _pointName)
{
    std::cout << _pointName << " --- x : " << _point.x() << " , y : " << _point.y() << "\n";
}

std::vector<QPixmap> Function::GetQPixmapVector(const std::vector<QString>& nameVec)
{
    std::vector<QPixmap> _ret;
    for (auto& _name : nameVec)
    {
        _ret.emplace_back<QPixmap>(QPixmap(_name));
    }
